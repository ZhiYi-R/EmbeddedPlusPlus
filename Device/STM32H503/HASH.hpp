...
#ifndef EMBEDDED_PP_STM32H503_HASH_HPP
#define EMBEDDED_PP_STM32H503_HASH_HPP
...
namespace STM32H503::HASH {
     ...
     *          - B_0x1: HMAC mode selected. LKEY bit must be set if the key being used is longer than 64 bytes. (value: 1)
     */
        /** @brief Hash mode selected */
    constexpr std::uint32_t HASH_CR_MODE_B_0x0 = 0;
        /** @brief HMAC mode selected. LKEY bit must be set if the key being used is longer than 64 bytes. */
    ...
    /** @brief Long key selection The application must set this bit if the HMAC key is greater than the block size (64 bytes) This selection is only taken into account when the INIT and MODE bits are set (HMAC mode selected). Changing this bit during a computation has no effect. */
    ...
    /** @details Possible values:
     *          - B_0x0: SHA-1 (value: 0)
     *          - B_0x2: SHA-224 (value: 2)
     *          - B_0x3: SHA-256 (value: 3)
     */
        /** @brief SHA-1 */
    ...
    /** @brief Hash data x Refer to Section 24.7.4: HASH digest registers introduction. */
    ...
    /** @brief Context swap x Refer to Section 24.7.7: HASH context swap registers introduction. */
