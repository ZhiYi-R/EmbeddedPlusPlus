...
#ifndef EMBEDDED_PP_STM32H503_TAMP_HPP
#define EMBEDDED_PP_STM32H503_TAMP_HPP
...
namespace STM32H503::TAMP {
    ...
    /** @details Possible values:
     *          - B_0x0: Internal Tamper 2 event erases the backup registers and all device secretssup(1)/sup. (value: 0)
     *          - B_0x1: Internal Tamper 2 event does not erase the backup registers and device secretssup(2)/sup. (value: 1)
     */
        /** @brief Internal Tamper 2 event erases the backup registers and all device secretssup(1)/sup. */
    constexpr std::uint32_t TAMP_CR3_ITAMP2NOER_B_0x0 = 0;
        /** @brief Internal Tamper 2 event does not erase the backup registers and device secretssup(2)/sup. */
    ...
    /** @details Possible values:
     *          - B_0x0: Internal Tamper 4 event erases the backup registers and all device secretssup(1)/sup. (value: 0)
     *          - B_0x1: Internal Tamper 4 event does not erase the backup registers and device secretssup(2)/sup. (value: 1)
     */
        /** @brief Internal Tamper 4 event erases the backup registers and all device secretssup(1)/sup. */
    constexpr std::uint32_t TAMP_CR3_ITAMP4NOER_B_0x0 = 0;
        /** @brief Internal Tamper 4 event does not erase the backup registers and device secretssup(2)/sup. */
    ...
    /** @details Possible values:
     *          - B_0x0: Internal Tamper 15 event erases the backup registers and all device secretssup(1)/sup. (value: 0)
     *          - B_0x1: Internal Tamper 15 event does not erase the backup registers and device secretssup(2)/sup. (value: 1)
     */
        /** @brief Internal Tamper 15 event erases the backup registers and all device secretssup(1)/sup. */
    constexpr std::uint32_t TAMP_CR3_ITAMP15NOER_B_0x0 = 0;
        /** @brief Internal Tamper 15 event does not erase the backup registers and device secretssup(2)/sup. */
     ...
     *          - B_0x1: RTCCLK/2 is selected when (PREDIV_A+1) = 128 (actually selects 1supst/sup flip flop output) (value: 1)
     *          - B_0x2: RTCCLK/4 is selected when (PREDIV_A+1) = 128 (actually selects 2supnd/sup flip flop output) (value: 2)
     *          - B_0x7: RTCCLK/128 is selected when (PREDIV_A+1) = 128 (actually selects 7supth/sup flip flop output) (value: 7)
     */
        /** @brief RTCCLK is selected */
    constexpr std::uint32_t TAMP_ATCR1_ATCKSEL_B_0x0 = 0;
        /** @brief RTCCLK/2 is selected when (PREDIV_A+1) = 128 (actually selects 1supst/sup flip flop output) */
    constexpr std::uint32_t TAMP_ATCR1_ATCKSEL_B_0x1 = 1;
        /** @brief RTCCLK/4 is selected when (PREDIV_A+1) = 128 (actually selects 2supnd/sup flip flop output) */
    constexpr std::uint32_t TAMP_ATCR1_ATCKSEL_B_0x2 = 2;
        /** @brief RTCCLK/128 is selected when (PREDIV_A+1) = 128 (actually selects 7supth/sup flip flop output) */
    constexpr std::uint32_t TAMP_ATCR1_ATCKSEL_B_0x7 = 7;

    /** @brief Active tamper output change period The tamper output is changed every CK_ATPER = (2supATPER /supx CK_ATPRE) cycles. Refer to Table 239: Minimum ATPER value. */
    ...
    /** @brief Backup registers read/write protection offset Protection zone 1 is defined for backup registers from TAMP_BKP0R to TAMP_BKPxR (x = BKPRW-1, from 0 to 128). Note: If BKPRW = 0: there is no protection zone 1. Note: If BKPRWPRIV is set, BKPRW[7:0] can be written only in privileged mode. */
    using TAMP_CFGR_BKPRW = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 8, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Backup registers write protection offset Protection zone 2 is defined for backup registers from TAMP_BKPyR (y = BKPRW, from 0 to 128) to TAMP_BKPzR (z = BKPW-1, from 0 to 128, BKPW greater than or equal BKPRW): Protection zone 3 defined for backup registers from TAMP_BKPtR (t = BKPW, from 0 to 127).. */
    ...
    /** @brief Tamper privilege protection (excluding backup registers) Note: Refer to Section 32.3.6: TAMP privilege protection modes for details on the read protection. */
    ...
    /** @brief internal tamper 12 interrupt masked flag This flag is set by hardware when the internal tamper 12 interrupt is raised. */
    using TAMP_MISR_ITAMP12MF = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 27, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief internal tamper 13 interrupt masked flag This flag is set by hardware when the internal tamper 13 interrupt is raised. */
    ...
    /** @brief The application can write or read data to and from these registers. In the default (ERASE) configuration this register is reset on a tamper detection event. It is forced to reset value as long as there is at least one internal or external tamper flag being set. */
