...
#ifndef EMBEDDED_PP_STM32H562_SEC_RNG_HPP
#define EMBEDDED_PP_STM32H562_SEC_RNG_HPP
...
namespace STM32H562::SEC_RNG {
    ...
    /** @brief Non NIST compliant four conditioning loops are performed and 512 bits of noise source are used. Writing this bit is taken into account only if CONDRST bit is set to 1 in the same access, while CONFIGLOCK remains at 0. Writing to this bit is ignored if CONFIGLOCK=1. */
     ...
     *          - B_0x1: Custom values for NIST compliant RNG. See Section 38.6: RNG entropy source validation for proposed configuration. (value: 1)
     */
    ...
    constexpr std::uint32_t RNG_CR_NISTC_B_0x0 = 0;
        /** @brief Custom values for NIST compliant RNG. See Section 38.6: RNG entropy source validation for proposed configuration. */
     ...
     *          - B_0xF: 215 RNG clock cycles per internal clock, e.g. an incoming 48MHz RNG clock becomes a 1.5 kHz internal RNG clock. (value: 15)
     */
    ...
    constexpr std::uint32_t RNG_CR_CLKDIV_B_0x1 = 1;
        /** @brief 215 RNG clock cycles per internal clock, e.g. an incoming 48MHz RNG clock becomes a 1.5 kHz internal RNG clock. */
    ...
    /** @details Possible values:
     *          - B_0x0: Writes to the RNG_CR configuration bits [29:4] are allowed. (value: 0)
     *          - B_0x1: Writes to the RNG_CR configuration bits [29:4] are ignored until the next RNG reset. (value: 1)
     */
        /** @brief Writes to the RNG_CR configuration bits [29:4] are allowed. */
    constexpr std::uint32_t RNG_CR_CONFIGLOCK_B_0x0 = 0;
        /** @brief Writes to the RNG_CR configuration bits [29:4] are ignored until the next RNG reset. */
    ...
    /** @brief Random data 32-bit random data which are valid when DRDY=1. When DRDY=0 RNDATA value is zero. It is recommended to always verify that RNG_DR is different from zero. Because when it is the case a seed error occurred between RNG_SR polling and RND_DR output reading (rare event). */
    ...
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 1. */
    using RNG_NSCR_EN_OSC1 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 2. */
    using RNG_NSCR_EN_OSC2 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 3, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 3. */
    using RNG_NSCR_EN_OSC3 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 6, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 4. */
    using RNG_NSCR_EN_OSC4 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 9, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 5. */
    using RNG_NSCR_EN_OSC5 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 12, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 6. */
    ...
    /** @brief health test configuration This configuration is used by RNG to configure the health tests. See entropy source validation for the recommended value. Note: The RNG behavior, including the read to this register, is not guaranteed if a different value from the recommended value is written. */
