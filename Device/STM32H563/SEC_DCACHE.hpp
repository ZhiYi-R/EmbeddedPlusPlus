...
#ifndef EMBEDDED_PP_STM32H563_SEC_DCACHE_HPP
#define EMBEDDED_PP_STM32H563_SEC_DCACHE_HPP
...
/** @brief Data cache */
namespace STM32H563::SEC_DCACHE {
    ...
    /** @brief full cache invalidation Can be set by software, only when EN = 1. Cleared by hardware when the BUSYF flag is set (during full cache invalidation operation). Writing 0 has no effect. */
    ...
    /** @brief cache command maintenance operation (cleans and/or invalidates an address range) Can be set and cleared by software, only when no maintenance command is ongoing (BUSYCMDF = 0). others: reserved */
    ...
    /** @brief starts maintenance command (maintenance operation defined in CACHECMD). Can be set by software, only when EN = 1, BUSYCMDF = 0, BUSYF = 0 and CACHECMD = 0b001, 0b010 or 0b011. Cleared by hardware when the BUSYCMDF flag is set (during cache maintenance operation). Writing 0 has no effect. */
    ...
    /** @brief output burst type for cache master port read accesses Write access is always done in INCR burst type. */
    ...
    /** @brief full invalidate busy end flag Cleared by writing DCACHE_FCR.CBSYENDF = 1. */
    ...
    /** @brief cache error flag Cleared by writing DCACHE_FCR.CERRF = 1. */
    ...
    /** @brief command end flag Cleared by writing DCACHE_FCR.CCMDENDF = 1. */
    ...
    /** @brief interrupt enable on busy end Set by SW to enable an interrupt generation at the end of a cache full invalidate operation. */
    ...
    /** @brief interrupt enable on cache error Set by software to enable an interrupt generation in case of cache functional error (eviction or clean operation write-back error) */
    ...
    /** @brief interrupt enable on command end Set by software to enable an interrupt generation at the end of a cache command (clean and/or invalidate an address range) */
    ...
    /** @brief clear full invalidate busy end flag Set by software. */
    ...
    /** @brief clear cache error flag Set by software. */
    ...
    /** @brief clear command end flag Set by software. */
    ...
    /** @brief start address of range to which the cache maintenance command specified in DCACHE_CR.CACHECMD field applies This register must be set before DCACHE_CR.CACHECMD is written. . */
    ...
    /** @brief end address of range to which the cache maintenance command specified in DCACHE_CR.CACHECMD field applies This register must be set before DCACHE_CR.CACHECMD is written. */
