...
#ifndef EMBEDDED_PP_STM32H563_SEC_RTC_HPP
#define EMBEDDED_PP_STM32H563_SEC_RTC_HPP
...
namespace STM32H563::SEC_RTC {
    ...
    /** @brief RTC_REFIN reference clock detection enable (50 or 60 Hz) Note: BIN must be 0x00 and PREDIV_S must be 0x00FF. */
    ...
    /** @brief Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768 kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to . */
    ...
    /** @details Possible values:
     *          - B_0x0: Calibration window is 220 RTCCLK, which is a high-consumption mode. This mode should be set only when less than 32s calibration window is required. (value: 0)
     *          - B_0x1: Calibration window is 220 ck_apre, which is the required configuration for ultra-low consumption mode. (value: 1)
     */
        /** @brief Calibration window is 220 RTCCLK, which is a high-consumption mode. This mode should be set only when less than 32s calibration window is required. */
    constexpr std::uint32_t RTC_CALR_LPCAL_B_0x0 = 0;
        /** @brief Calibration window is 220 ck_apre, which is the required configuration for ultra-low consumption mode. */
     ...
     *          - B_0x1: One RTCCLK pulse is effectively inserted every 211 pulses (frequency increased by 488.5 ppm). (value: 1)
     */
        /** @brief No RTCCLK pulses are added. */
    constexpr std::uint32_t RTC_CALR_CALP_B_0x0 = 0;
        /** @brief One RTCCLK pulse is effectively inserted every 211 pulses (frequency increased by 488.5 ppm). */
    ...
    /** @brief Sub second value/Synchronous binary counter values SS[31:0] is the value of the synchronous prescaler counter when the timestamp event occurred. */
     ...
     *          - B_0x1: The synchronous binary counter (SS[31:0] in RTC_SSR) is running from 0xFFFF FFFF to RTC_ALRMABINR.SS[31:0] value and is automatically reloaded with 0xFFFF FFFF one ck_apre cycle after reaching RTC_ALRMABINR.SS[31:0]. (value: 1)
     */
        /** @brief The synchronous binary counter (SS[31:0] in RTC_SSR) is free-running. */
    constexpr std::uint32_t RTC_ALRMASSR_SSCLR_B_0x0 = 0;
        /** @brief The synchronous binary counter (SS[31:0] in RTC_SSR) is running from 0xFFFF FFFF to RTC_ALRMABINR.SS[31:0] value and is automatically reloaded with 0xFFFF FFFF one ck_apre cycle after reaching RTC_ALRMABINR.SS[31:0]. */
    ...
    /** @brief Sub seconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm B is to be activated. Only bits 0 up to MASKSS-1 are compared. This field is the mirror of SS[14:0] in the RTC_ALRMBBINR, and so can also be read or written through RTC_ALRMBBINR. */
    ...
    /** @details Possible values:
     *          - B_0x0: The synchronous binary counter (SS[31:0] in RTC_SSR) is free-running. (value: 0)
     *          - B_0x1: The synchronous binary counter (SS[31:0] in RTC_SSR) is running from 0xFFFF FFFF to RTC_ALRMBBINR.SS[31:0] value and is automatically reloaded with 0xFFFF FFFF one ck_apre cycle after reaching RTC_ALRMBBINR.SS[31:0]. (value: 1)
     */
        /** @brief The synchronous binary counter (SS[31:0] in RTC_SSR) is free-running. */
    constexpr std::uint32_t RTC_ALRMBSSR_SSCLR_B_0x0 = 0;
        /** @brief The synchronous binary counter (SS[31:0] in RTC_SSR) is running from 0xFFFF FFFF to RTC_ALRMBBINR.SS[31:0] value and is automatically reloaded with 0xFFFF FFFF one ck_apre cycle after reaching RTC_ALRMBBINR.SS[31:0]. */
    ...
    /** @brief Timestamp flag This flag is set by hardware when a timestamp event occurs. If ITSF flag is set, TSF must be cleared together with ITSF. Note: TSF is not set if TAMPTS = 1 and the tamper flag is read during the 3 ck_apre cycles following tamper event. Refer to for more details. */
    ...
    /** @brief Timestamp non-secure masked flag This flag is set by hardware when a timestamp non-secure interrupt occurs. If ITSF flag is set, TSF must be cleared together with ITSF. */
    ...
    /** @brief Internal timestamp non-secure masked flag This flag is set by hardware when a timestamp on the internal event occurs and timestamp non-secure interrupt is raised. */
    ...
    /** @brief Timestamp interrupt secure masked flag This flag is set by hardware when a timestamp secure interrupt occurs. If ITSF flag is set, TSF must be cleared together with ITSF. */
    ...
    /** @brief Internal timestamp interrupt secure masked flag This flag is set by hardware when a timestamp on the internal event occurs and timestamp secure interrupt is raised. */
    ...
    using RTC_OR = ::EmbeddedPP::LowLevel::Register<std::uint32_t, 0x60, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief RTC_OUT2 mapping */
    using RTC_OR_OUT2_RMP = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @details Possible values:
     *          - B_0x0: RTC_OUT2 is mapped on PI8 (value: 0)
     *          - B_0x1: RTC_OUT2 is mapped on PB2 (value: 1)
     */
        /** @brief RTC_OUT2 is mapped on PI8 */
    constexpr std::uint32_t RTC_OR_OUT2_RMP_B_0x0 = 0;
        /** @brief RTC_OUT2 is mapped on PB2 */
    constexpr std::uint32_t RTC_OR_OUT2_RMP_B_0x1 = 1;
