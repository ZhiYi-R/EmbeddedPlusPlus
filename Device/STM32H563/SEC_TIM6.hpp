...
#ifndef EMBEDDED_PP_STM32H563_SEC_TIM6_HPP
#define EMBEDDED_PP_STM32H563_SEC_TIM6_HPP
...
namespace STM32H563::SEC_TIM6 {
    ...
    /** @details Possible values:
     *          - B_0x0: Resetthe UG bit from the TIMx_EGR register is used as a trigger output (tim_trgo). (value: 0)
     *          - B_0x1: Enablethe Counter enable signal, tim_cnt_en, is used as a trigger output (tim_trgo). It is useful to start several timers at the same time or to control a window in which a slave timer is enabled. The Counter Enable signal is generated when the CEN control bit is written. (value: 1)
     *          - B_0x2: UpdateThe update event is selected as a trigger output (tim_trgo). For instance a master timer can then be used as a prescaler for a slave timer. (value: 2)
     */
        /** @brief Resetthe UG bit from the TIMx_EGR register is used as a trigger output (tim_trgo). */
    constexpr std::uint16_t TIM6_CR2_MMS_B_0x0 = 0;
        /** @brief Enablethe Counter enable signal, tim_cnt_en, is used as a trigger output (tim_trgo). It is useful to start several timers at the same time or to control a window in which a slave timer is enabled. The Counter Enable signal is generated when the CEN control bit is written. */
    constexpr std::uint16_t TIM6_CR2_MMS_B_0x1 = 1;
        /** @brief UpdateThe update event is selected as a trigger output (tim_trgo). For instance a master timer can then be used as a prescaler for a slave timer. */
