...
#ifndef EMBEDDED_PP_STM32H573_SEC_OTFDEC1_HPP
#define EMBEDDED_PP_STM32H573_SEC_OTFDEC1_HPP
...
namespace STM32H573::SEC_OTFDEC1 {
    ...
    /** @brief Privileged access protection. Unprivileged read accesses to registers return zeros Unprivileged write accesses to registers are ignored. Note: This bit can only be written in privileged mode. There is no limitations on reads. */
    ...
    /** @brief region on-the-fly decryption enable Note: Garbage is decrypted if region context (version, key, nonce) is not valid when this bit is set. */
    ...
    /** @brief region config lock Note: This bit is set once. If this bit is set, it can only be reset to 0 if OTFDEC is reset. Setting this bit forces KEYLOCK bit to 1. */
    ...
    /** @brief region key lock Note: This bit is set once: if this bit is set, it can only be reset to 0 if the OTFDEC is reset. */
    ...
    /** @brief operating mode This bitfield selects the OTFDEC operating mode for this region: Others: Reserved When MODE different 11, the standard AES encryption mode is activated. When either of the MODE bits are changed, the region key and associated CRC are zeroed. */
    ...
    /** @brief region firmware version This 16-bit bitfield must be correctly initialized before the region corresponding REG_EN bit is set in OTFDEC_RxCFGR. */
    ...
    /** @brief Region AHB start address This register must be written before the region corresponding REG_EN bit in the OTFDEC_RxCFGR register is set. Writing to this register is discarded if performed while the region CONFIGLOCK bit in the OTFDEC_RxCFGR register is set. Note: When determining the region the first 12 bits (lsb) and the last 4 bits (msb) are ignored. When this register is accessed in read the 4 msb bits and the 12 lsb bits return zeros . */
    ...
    /** @brief Region nonce, bits [31:0] This register must be written before the region corresponding REG_EN bit in OTFDEC_RxCFGR is set. Writing is discarded in this register if performed while the region CONFIGLOCK bit in the OTFDEC_RxCFGR is set. */
    ...
    /** @brief Region key, bits [31:0] This register must be written before the region corresponding REG_EN bit in OTFDEC_RxCFGR is set. Reading this register returns a zero value. Writing to this register is discarded if performed while the region CONFIGLOCK or KEYLOCK bit is set in the OTFDEC_RxCFGR. Note: When application successfully changes MODE bits in OTFDEC_RxCFGR and OTFDEC_RxKEYR, and associated KEYCRC are erased. */
    ...
    /** @brief Security error interrupt flag status This bit is set by hardware and read only by application. This bit is set when at least one security error has been detected. This bit is cleared when application sets in OTFDEC_ICR the corresponding bit to 1. */
     ...
     *          - B_0x1: Read access detected on one region with MODE bits set to 11 or execute access detected while ENC = 1. OTFDEC returns a zeroed value for the illegal access, and an optional interrupt is generated if bit XONEIE is set to 1 in OTFDEC_IER. (value: 1)
     */
        /** @brief No execute-only error status. No interrupt pending. */
    constexpr std::uint32_t OTFDEC_ISR_XONEIF_B_0x0 = 0;
        /** @brief Read access detected on one region with MODE bits set to 11 or execute access detected while ENC = 1. OTFDEC returns a zeroed value for the illegal access, and an optional interrupt is generated if bit XONEIE is set to 1 in OTFDEC_IER. */
     ...
     *          - B_0x1: Read access detected on an enabled encrypted region with its key registers null or not properly initialized (KEYCRC = 0x0). OTFDEC returns a zeroed value for the read, and an optional interrupt is generated if bit KEIE is set to 1 in OTFDEC_IER. (value: 1)
     */
        /** @brief OTFDEC operates properly. */
    constexpr std::uint32_t OTFDEC_ISR_KEIF_B_0x0 = 0;
        /** @brief Read access detected on an enabled encrypted region with its key registers null or not properly initialized (KEYCRC = 0x0). OTFDEC returns a zeroed value for the read, and an optional interrupt is generated if bit KEIE is set to 1 in OTFDEC_IER. */
    ...
    /** @brief Security error interrupt flag clear This bit is written by application, and always read as 0. */
    ...
    /** @brief Execute-only execute-never error interrupt flag clear This bit is written by application, and always read as 0. */
    ...
    /** @brief Key error interrupt flag clear This bit is written by application, and always read as 0. Note: Clearing KEIF does not solve the source of the problem (bad key registers). To be able to access again any encrypted region, OTFDEC key registers must be properly initialized again. */
