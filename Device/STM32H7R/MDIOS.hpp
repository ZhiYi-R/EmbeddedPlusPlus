...
#ifndef EMBEDDED_PP_STM32H7R_MDIOS_HPP
#define EMBEDDED_PP_STM32H7R_MDIOS_HPP
...
namespace STM32H7R::MDIOS {
    ...
    /** @brief register write interrupt enable Note: When this bit is set, an interrupt is generated if any of the read flags (WRF[31:0] in the MDIOS_WRFR register) is set. */
    ...
    /** @brief register read interrupt enable Note: When this bit is set, an interrupt is generated if any of the read flags (RDF[31:0] in the MDIOS_RDFR register) is set. */
    ...
    /** @brief error interrupt enable Note: When this bit is set, an interrupt is generated if any of the error flags (PERF, SERF, or TERF in the MDIOS_SR register) is set. */
    ...
    /** @brief slave address Can be written only when the peripheral is disabled (EN = 0). If the address given by the MDIO master matches PORT_ADRESS[4:0], then the MDIOS services the frame. Otherwise the frame is ignored. */
    ...
    /** @brief clear the write flag Writing 1 to CWRFx clears the WRFx bit in the MDIOS_WRF register. */
    ...
    /** @brief clear the read flag Writing 1 to CRDFx clears the RDFx bit in the MDIOS_RDF register. */
    ...
    /** @brief preamble error flag Note: Writing 1 to CPERF (MDIOS_CLRFR) clears this bit. Note: This bit is not set if DPC (disable preamble check, MDIOS_CR[7]) is set. */
    ...
    /** @brief start error flag Note: Writing 1 to CSERF (MDIOS_CLRFR) clears this bit. */
    ...
    /** @brief turnaround error flag Note: Writing 1 to CTERF (MDIOS_CLRFR) clears this bit. */
    ...
    /** @brief clear the preamble error flag Writing 1 to this bit clears the PERF flag (in MDIOS_SR). */
    using MDIOS_CLRFR_CPERF = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief clear the start error flag Writing 1 to this bit clears the SERF flag (in MDIOS_SR). When DPC = 1 (MDIOS_CR[7]), the SERF flag must be cleared only when there is not a frame already in progress. */
    using MDIOS_CLRFR_CSERF = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 1, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief clear the turnaround error flag Writing 1 to this bit clears the TERF flag (in MDIOS_SR). When DPC = 1 (MDIOS_CR[7]), the TERF flag must be cleared only when there is not a frame already in progress. */
    ...
    /** @brief input data received from MDIO master during write frames This field is written by hardware with the 16-bit data received in a write frame which is addressed to MDIOS register x. */
    ...
    /** @brief output data sent to MDIO Master during read frames This field is written by software. These 16 bits are serially output on the MDIO bus during read frames which address the MDIOS register x. */
