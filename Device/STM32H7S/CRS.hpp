...
#ifndef EMBEDDED_PP_STM32H7S_CRS_HPP
#define EMBEDDED_PP_STM32H7S_CRS_HPP
...
namespace STM32H7S::CRS {
    ...
    /** @brief Automatic trimming enable This bit enables the automatic hardware adjustment of TRIM bits according to the measured frequency error between two SYNC events. If this bit is set, the TRIM bits are read-only. The TRIM value can be adjusted by hardware by one or two steps at a time, depending on the measured frequency error value. Refer to Section 7.5.3 for more details. */
    ...
    /** @brief Counter reload value RELOAD is the value to be loaded in the frequency error counter with each SYNC event. Refer to Section 7.5.2 for more details about counter behavior. */
    using CRS_CFGR_RELOAD = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 16, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Frequency error limit FELIM contains the value to be used to evaluate the captured frequency error value latched in the FECAP[15:0] bits of the CRS_ISR register. Refer to Section 7.5.3 for more details about FECAP evaluation. */
    ...
    /** @details Possible values:
     *          - B_0x0: CRS_SYNC pin selected as SYNC signal source (value: 0)
     *          - B_0x1: LSE selected as SYNC signal source (value: 1)
     *          - B_0x2: OTG HS SOF selected as SYNC signal source (default) (value: 2)
     *          - B_0x3: FIELD Reserved (value: 3)
     */
        /** @brief CRS_SYNC pin selected as SYNC signal source */
    constexpr std::uint32_t CRS_CFGR_SYNCSRC_B_0x0 = 0;
        /** @brief LSE selected as SYNC signal source */
    constexpr std::uint32_t CRS_CFGR_SYNCSRC_B_0x1 = 1;
        /** @brief OTG HS SOF selected as SYNC signal source (default) */
    ...
    /** @brief Frequency error capture FECAP is the frequency error counter value latched in the time of the last SYNC event. Refer to Section 7.5.3 for more details about FECAP usage. */
