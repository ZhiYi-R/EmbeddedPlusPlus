...
#ifndef EMBEDDED_PP_STM32H7S_SAES_HPP
#define EMBEDDED_PP_STM32H7S_SAES_HPP
...
namespace STM32H7S::SAES {
    ...
    constexpr std::uint32_t SAES_CR_MODE_B_0x3 = 3;

    /** @brief CHMOD[1:0]: Chaining mode selection This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when the BUSY flag of SAES_SR register is set, as well as when the EN bit of the SAES_CR register is set before the write access and it is not cleared by that write access. */
     ...
     *          - B_0x3: Application hardware key (AHK) (value: 3)
     *          - B_0x4: XOR of DHUK and BHK (value: 4)
     *          - B_0x5: XOR of DHUK and AHK (value: 5)
    ...
    constexpr std::uint32_t SAES_CR_KEYSEL_B_0x2 = 2;
        /** @brief Application hardware key (AHK) */
    constexpr std::uint32_t SAES_CR_KEYSEL_B_0x3 = 3;
        /** @brief XOR of DHUK and BHK */
    constexpr std::uint32_t SAES_CR_KEYSEL_B_0x4 = 4;
        /** @brief XOR of DHUK and AHK */
    constexpr std::uint32_t SAES_CR_KEYSEL_B_0x5 = 5;
    ...
    /** @brief Initialization vector input, bits [31:0] Refer to Section 32.4.17: SAES initialization vector registers on page 1760 for description of the IVI[127:0] bitfield. The initialization vector is only used in chaining modes other than ECB. The SAES_IVRx registers may be written only when the SAES peripheral is disabled */
