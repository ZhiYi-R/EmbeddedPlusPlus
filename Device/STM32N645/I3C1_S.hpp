...
#ifndef EMBEDDED_PP_STM32N645_I3C1_S_HPP
#define EMBEDDED_PP_STM32N645_I3C1_S_HPP
...
namespace STM32N645::I3C1_S {
    ...
    /** @brief 7-bit I3C dynamic / I less than sup>2 less than /sup>C static target address (when I3C acts as controller) */
     ...
     *          - B_0x2_WHEN_I3C_ACTS_AS_I3C_CONTROLLER: private message (refer to Figure 801) (value: 2)
     *          - B_0x3_WHEN_I3C_ACTS_AS_I3C_CONTROLLER: direct message (second part of an I3C SDR direct CCC command) (refer to Figure 794) (value: 3)
     *          - B_0x4_WHEN_I3C_ACTS_AS_I3C_CONTROLLER: legacy I less than sup>2 less than /sup>C message (refer to Figure 803) (value: 4)
     *          - B_0x8_WHEN_I3C_ACTS_AS_I3C_TARGET: hot-join request (W) (refer to Figure 805) (value: 8)
     *          - B_0x9_WHEN_I3C_ACTS_AS_I3C_TARGET: controller-role request (W) (refer to Figure 806) (value: 9)
     *          - B_0xA_WHEN_I3C_ACTS_AS_I3C_TARGET: IBI (in-band interrupt) request (R) (refer to Figure 804) (value: 10)
     */
     ...
     */
        /** @brief no data to transfer. */
    constexpr std::uint32_t I3C_CR_alternate_DCNT_B_0x0000 = 0;
        /** @brief 1 byte */
    constexpr std::uint32_t I3C_CR_alternate_DCNT_B_0x0001 = 1;
        /** @brief 2 bytes */
    constexpr std::uint32_t I3C_CR_alternate_DCNT_B_0x0002 = 2;
        /** @brief 64 Kbytes - 1 byte */
    ...
    /** @details Possible values:
     *          - B_0x6_WHEN_I3C_ACTS_AS_I3C_CONTROLLER: broadcast/direct CCC command (refer to Table 621, Figure 794, Figure 795, Figure 796) (value: 6)
     */
        /** @brief broadcast/direct CCC command (refer to Table 621, Figure 794, Figure 795, Figure 796) */
    constexpr std::uint32_t I3C_CR_alternate_MTYPE_B_0x6_WHEN_I3C_ACTS_AS_I3C_CONTROLLER = 6;
    ...
    /** @details Possible values:
     *          - B_0x0: An arbitrable header (0b111_1110 + RnW = 0) is emitted after a start and before a legacy I less than sup>2 less than /sup>C message or an I3C SDR private read/write message (default). (value: 0)
     *          - B_0x1: No arbitrable header (value: 1)
     */
        /** @brief An arbitrable header (0b111_1110 + RnW = 0) is emitted after a start and before a legacy I less than sup>2 less than /sup>C message or an I3C SDR private read/write message (default). */
    ...
    /** @details Possible values:
     *          - B_0x0: HDR exit pattern is not sent after the issued message header (MTYPE[3:0] = 0001 in the I3C_CR register). This is used to send the header, to test ownership of the bus when there is a suspicion of a problem after controller-role hand-off (new controller did not assert its controller-role by accessing the previous one in less than the delay defined by the activity state). (value: 0)
     *          - B_0x1: HDR exit pattern is sent after the issued message header (MTYPE[3:0] = 0001). This is used on a controller error detection and escalation handling, in case of a not responding target to a private message or a direct read CCC. (value: 1)
     */
        /** @brief HDR exit pattern is not sent after the issued message header (MTYPE[3:0] = 0001 in the I3C_CR register). This is used to send the header, to test ownership of the bus when there is a suspicion of a problem after controller-role hand-off (new controller did not assert its controller-role by accessing the previous one in less than the delay defined by the activity state). */
    constexpr std::uint32_t I3C_CFGR_EXITPTRN_B_0x0 = 0;
        /** @brief HDR exit pattern is sent after the issued message header (MTYPE[3:0] = 0001). This is used on a controller error detection and escalation handling, in case of a not responding target to a private message or a direct read CCC. */
     ...
     *          - B_0x1: C-FIFO and TX-FIFO are first preloaded (also TX-FIFO if needed, depending on the frame format) before starting to emit a frame transfer. Refer to Section 64.10.2 for more details. (value: 1)
     */
    ...
    constexpr std::uint32_t I3C_CFGR_TMODE_B_0x0 = 0;
        /** @brief C-FIFO and TX-FIFO are first preloaded (also TX-FIFO if needed, depending on the frame format) before starting to emit a frame transfer. Refer to Section 64.10.2 for more details. */
    ...
    /** @brief C-FIFO not full interrupt enable (whatever the I3C acts as controller/target) */
    ...
    /** @brief S-FIFO not empty interrupt enable (whatever the I3C acts as controller/target) */
     ...
     *          - B_0x2: second level of reset: the application software must issue a warm reset, also known as a system reset. This (see Section 21.3: RCC functional description - Reset) has the same impact as a pin reset (NRST = 0): (value: 2)
    ...
    constexpr std::uint32_t I3C_DEVR0_RSTACT_B_0x1 = 1;
        /** @brief second level of reset: the application software must issue a warm reset, also known as a system reset. This (see Section 21.3: RCC functional description - Reset) has the same impact as a pin reset (NRST = 0): */
    ...
    /** @details Possible values:
     *          - B_0x0: null payload data size (only allowed when BCR2 = 0 in the IC3_BCR register) (value: 0)
     ...
     */
        /** @brief null payload data size (only allowed when BCR2 = 0 in the IC3_BCR register) */
    ...
    /** @brief SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles: */
    using I3C_TIMINGR0_SCLL_OD = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 16, 8, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles: */
    ...
    /** @brief SDA hold time (when the I3C acts as controller), in number of kernel clocks cycles (refer to MIPI timing SDA hold time in push-pull t less than sub>HD_PP less than /sub>): */
    ...
    /** @brief Controller clock stall enable on T-bit phase of data (and on the ACK/NACK phase of data byte of a legacy I less than sup>2 less than /sup>C read) */
    ...
    /** @details Possible values:
     *          - B_0x00: generic device (for v1.0 devices) (value: 0)
     */
     ...
     *          - B_0x1: format 2: (5 bytes w.ith MaxWr with no defining byte, MaxRd, MaxRdTurn) (value: 1)
    ...
    constexpr std::uint32_t I3C_GETMXDSR_FMT_B_0x0 = 0;
        /** @brief format 2: (5 bytes w.ith MaxWr with no defining byte, MaxRd, MaxRdTurn) */
    ...
    /** @brief clock-to-data turnaround time (t less than sub>SCO less than /sub>) */
    using I3C_GETMXDSR_TSCO = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 24, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @details Possible values:
     *          - B_0x0: t less than sub>SCO less than /sub> less than or equal 12 ns (value: 0)
     *          - B_0x1: t less than sub>SCO less than /sub> > 12 ns (refer to the datasheet for more details) (value: 1)
     */
        /** @brief t less than sub>SCO less than /sub> less than or equal 12 ns */
    constexpr std::uint32_t I3C_GETMXDSR_TSCO_B_0x0 = 0;
        /** @brief t less than sub>SCO less than /sub> > 12 ns (refer to the datasheet for more details) */
