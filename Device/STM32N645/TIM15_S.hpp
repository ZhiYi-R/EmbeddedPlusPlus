...
#ifndef EMBEDDED_PP_STM32N645_TIM15_S_HPP
#define EMBEDDED_PP_STM32N645_TIM15_S_HPP
...
namespace STM32N645::TIM15_S {
     ...
     *          - B_0x1: The timer operation is synchronized with the ADC clock to provide jitter-free sampling point. This mode can be enabled only with specific ADC / timer clock relationship. Refer to Section 57.4.29 for requirements. (value: 1)
     */
        /** @brief The timer operates independently from the ADC */
    constexpr std::uint32_t TIM15_CR2_ADSYNC_B_0x0 = 0;
        /** @brief The timer operation is synchronized with the ADC clock to provide jitter-free sampling point. This mode can be enabled only with specific ADC / timer clock relationship. Refer to Section 57.4.29 for requirements. */
     ...
     *          - B_0x6: PWM mode 1 - Channel 1 is active as long as TIM15_CNT less than TIM15_CCR1 else inactive. (value: 6)
     *          - B_0x7: PWM mode 2 - Channel 1 is inactive as long as TIM15_CNT less than TIM15_CCR1 else active. (value: 7)
     */
        /** @brief Frozen - The comparison between the output compare register TIM15_CCR1 and the counter TIM15_CNT has no effect on the outputs. This mode can be used when the timer serves as a software timebase. When the frozen mode is enabled during timer operation, the output keeps the state (active or inactive) it had before entering the frozen state. */
    ...
    constexpr std::uint32_t TIM15_CCMR1_OUTPUT_OC1M_B_0x5 = 5;
        /** @brief PWM mode 1 - Channel 1 is active as long as TIM15_CNT less than TIM15_CCR1 else inactive. */
    constexpr std::uint32_t TIM15_CCMR1_OUTPUT_OC1M_B_0x6 = 6;
        /** @brief PWM mode 2 - Channel 1 is inactive as long as TIM15_CNT less than TIM15_CCR1 else active. */
