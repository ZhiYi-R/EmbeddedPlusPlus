...
#ifndef EMBEDDED_PP_STM32N645_XSPI2_S_HPP
#define EMBEDDED_PP_STM32N645_XSPI2_S_HPP
...
namespace STM32N645::XSPI2_S {
     ...
     *          - B_0x4: HyperBus memory mode, the protocol follows the HyperBus less than sup> less than /sup>specification. 8-data-bit DTR mode must be selected. (value: 4)
    ...
    constexpr std::uint32_t XSPI_DCR1_MTYP_B_0x3 = 3;
        /** @brief HyperBus memory mode, the protocol follows the HyperBus less than sup> less than /sup>specification. 8-data-bit DTR mode must be selected. */
    ...
    /** @details Possible values:
     *          - B_0x0: F less than sub>CLK less than /sub> = F less than sub>KERNEL less than /sub>, kernel clock used directly as XSPI CLK (prescaler bypassed). In this case, if the DTR mode is used, it is mandatory to provide to the XSPI a kernel clock that has 50% duty-cycle. (value: 0)
     *          - B_0x1: F less than sub>CLK less than /sub> = F less than sub>KERNEL less than /sub>/2 (value: 1)
     */
        /** @brief F less than sub>CLK less than /sub> = F less than sub>KERNEL less than /sub>, kernel clock used directly as XSPI CLK (prescaler bypassed). In this case, if the DTR mode is used, it is mandatory to provide to the XSPI a kernel clock that has 50% duty-cycle. */
    constexpr std::uint32_t XSPI_DCR2_PRESCALER_B_0x0 = 0;
        /** @brief F less than sub>CLK less than /sub> = F less than sub>KERNEL less than /sub>/2 */
