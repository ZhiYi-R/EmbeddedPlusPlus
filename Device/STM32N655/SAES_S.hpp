...
#ifndef EMBEDDED_PP_STM32N655_SAES_S_HPP
#define EMBEDDED_PP_STM32N655_SAES_S_HPP
...
namespace STM32N655::SAES_S {
    ...
    /** @details Possible values:
     *          - B_0x0: Normal key mode. Key registers are freely usable and no specific use or protection applies to SAES_DINR and SAES_DOUTR registers. (value: 0)
     ...
     *          - B_0x2: Shared key mode. After a successful decryption process, SAES key registers are shared with the peripheral described in KSHAREID[1:0] bitfield. This sharing is valid only while KMOD[1:0] at 0x2 and KEYVALID=1. When a decryption is selected, read-as-zero SAES_DOUTR register is automatically loaded into SAES key registers after a successful decryption process. (value: 2)
     */
        /** @brief Normal key mode. Key registers are freely usable and no specific use or protection applies to SAES_DINR and SAES_DOUTR registers. */
    ...
    constexpr std::uint32_t SAES_CR_KMOD_B_0x1 = 1;
        /** @brief Shared key mode. After a successful decryption process, SAES key registers are shared with the peripheral described in KSHAREID[1:0] bitfield. This sharing is valid only while KMOD[1:0] at 0x2 and KEYVALID=1. When a decryption is selected, read-as-zero SAES_DOUTR register is automatically loaded into SAES key registers after a successful decryption process. */
    ...
    /** @details Possible values:
     *          - B_0x0: CRYP peripheral (value: 0)
     */
        /** @brief CRYP peripheral */
