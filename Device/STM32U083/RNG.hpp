...
#ifndef EMBEDDED_PP_STM32U083_RNG_HPP
#define EMBEDDED_PP_STM32U083_RNG_HPP
...
namespace STM32U083::RNG {
     ...
     *          - B_0x1: RNG interrupt is enabled. An interrupt is pending as soon as DRDY1=11, SEIS1=11 or CEIS1=11 in the RNG_SR register. (value: 1)
     */
        /** @brief RNG interrupt is disabled */
    constexpr std::uint32_t RNG_CR_IE_B_0x0 = 0;
        /** @brief RNG interrupt is enabled. An interrupt is pending as soon as DRDY1=11, SEIS1=11 or CEIS1=11 in the RNG_SR register. */
    ...
    /** @brief Auto reset disable When auto-reset is enabled the application still need to clear the SEIS bit after a noise source error. Writing this bit is taken into account only if CONDRST bit is set to 1 in the same access, while CONFIGLOCK remains at 0. Writing to this bit is ignored if CONFIGLOCK1=11. */
    ...
    /** @brief RNG configuration 3 Reserved to the RNG configuration (bitfield 3). Refer to RNG_CONFIG1 bitfield for details. If the NISTC bit is cleared in this register RNG_CONFIG3 bitfield values are ignored by RNG. */
    using RNG_CR_RNG_CONFIG3 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 8, 4, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief NIST custom two conditioning loops are performed and 256 bits of noise source are used. Writing this bit is taken into account only if CONDRST bit is set to 1 in the same access, while CONFIGLOCK remains at 0. Writing to this bit is ignored if CONFIGLOCK1=11. */
     ...
     *          - B_0x1: Custom values for NIST compliant RNG. See Section120.6: RNG entropy source validation for proposed configuration. (value: 1)
     */
    ...
    constexpr std::uint32_t RNG_CR_NISTC_B_0x0 = 0;
        /** @brief Custom values for NIST compliant RNG. See Section120.6: RNG entropy source validation for proposed configuration. */
    constexpr std::uint32_t RNG_CR_NISTC_B_0x1 = 1;

    /** @brief RNG configuration 2 Reserved to the RNG configuration (bitfield 2). Bit 13 can be set when RNG power consumption is critical. See Section120.3.8: RNG low-power use. Refer to the RNG_CONFIG1 bitfield for details. */
    using RNG_CR_RNG_CONFIG2 = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 13, 3, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Clock divider factor This value used to configure an internal programmable divider (from 1 to 16) acting on the incoming RNG clock. These bits can be written only when the core is disabled (RNGEN1=10). ... Writing these bits is taken into account only if the CONDRST bit is set to 1 in the same access, while CONFIGLOCK remains at 0. Writing to this bit is ignored if CONFIGLOCK1=11. */
     ...
     *          - B_0xF: 2<sup>15</sup> RNG clock cycles per internal clock (for example. an incoming 481MHz RNG clock becomes a 1.51kHz internal RNG clock) (value: 15)
     */
    ...
    constexpr std::uint32_t RNG_CR_CLKDIV_B_0x2 = 2;
        /** @brief 2<sup>15</sup> RNG clock cycles per internal clock (for example. an incoming 481MHz RNG clock becomes a 1.51kHz internal RNG clock) */
    ...
    /** @details Possible values:
     *          - B_0x0: The RNG clock is correct (f<sub>RNGCLK</sub>> f<sub>HCLK</sub>/32). If the CEIS bit is set, this means that a slow clock was detected and the situation has been recovered. (value: 0)
     *          - B_0x1: The RNG clock is too slow (f<sub>RNGCLK</sub>< f<sub>HCLK</sub>/32). (value: 1)
     */
        /** @brief The RNG clock is correct (f<sub>RNGCLK</sub>> f<sub>HCLK</sub>/32). If the CEIS bit is set, this means that a slow clock was detected and the situation has been recovered. */
    constexpr std::uint32_t RNG_SR_CECS_B_0x0 = 0;
        /** @brief The RNG clock is too slow (f<sub>RNGCLK</sub>< f<sub>HCLK</sub>/32). */
    constexpr std::uint32_t RNG_SR_CECS_B_0x1 = 1;

    /** @brief Seed error current status Runtime repetition count test failed (noise source has provided more than 24 consecutive bits at a constant value 0 or 1, or more than 32 consecutive occurrence of two bits patterns 01 or 10) Startup or continuous adaptive proportion test on noise source failed. Startup post-processing/conditioning sanity check failed. */
    ...
    /** @details Possible values:
     *          - B_0x0: The RNG clock is correct (f<sub>RNGCLK</sub>> f<sub>HCLK</sub>/32) (value: 0)
     *          - B_0x1: The RNG clock before the internal divider is detected too slow (f<sub>RNGCLK</sub>< f<sub>HCLK</sub>/32) (value: 1)
     */
        /** @brief The RNG clock is correct (f<sub>RNGCLK</sub>> f<sub>HCLK</sub>/32) */
    constexpr std::uint32_t RNG_SR_CEIS_B_0x0 = 0;
        /** @brief The RNG clock before the internal divider is detected too slow (f<sub>RNGCLK</sub>< f<sub>HCLK</sub>/32) */
