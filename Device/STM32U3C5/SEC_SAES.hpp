...
#ifndef EMBEDDED_PP_STM32U3C5_SEC_SAES_HPP
#define EMBEDDED_PP_STM32U3C5_SEC_SAES_HPP
...
namespace STM32U3C5::SEC_SAES {
    ...
    /** @details Possible values:
     *          - B_0x0: When KEYVALID is set and KEYSEL[2:0]=0 application can transfer the ownership of the SAES, with its loaded key, to an application running in another security context (such as nonsecure, secure). (value: 0)
     *          - B_0x1: When KEYVALID is set, key error flag (KEIF) is set when an access to any registers is detected, this access having a security context (for example, secure, nonsecure) that does not match the one of the application that loaded the key. (value: 1)
     */
        /** @brief When KEYVALID is set and KEYSEL[2:0]=0 application can transfer the ownership of the SAES, with its loaded key, to an application running in another security context (such as nonsecure, secure). */
    constexpr std::uint32_t SAES_CR_KEYPROT_B_0x0 = 0;
        /** @brief When KEYVALID is set, key error flag (KEIF) is set when an access to any registers is detected, this access having a security context (for example, secure, nonsecure) that does not match the one of the application that loaded the key. */
