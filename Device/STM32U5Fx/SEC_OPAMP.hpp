...
#ifndef EMBEDDED_PP_STM32U5FX_SEC_OPAMP_HPP
#define EMBEDDED_PP_STM32U5FX_SEC_OPAMP_HPP
...
namespace STM32U5Fx::SEC_OPAMP {
    ...
    /** @brief OPAMP low-power mode The OPAMP must be disabled to change this configuration. */
    ...
    /** @brief OPAMP PGA mode 00 and 01: internal PGA disabled */
    ...
    /** @brief Inverting input selection These bits are used only when OPAMODE = 00, 01 or 10. 1x: inverting input not externally connected */
     ...
     *          - B_0x1: PMOS calibration (V<sub>DDA </sub>- 200 mV applied on OPAMP inputs) (value: 1)
     */
        /** @brief NMOS calibration (200�mV applied on OPAMP inputs) */
    constexpr std::uint32_t OPAMP1_CSR_CALSEL_B_0x0 = 0;
        /** @brief PMOS calibration (V<sub>DDA </sub>- 200 mV applied on OPAMP inputs) */
    constexpr std::uint32_t OPAMP1_CSR_CALSEL_B_0x1 = 1;

    /** @brief ‘factory’ or ‘user’ offset trimmed values selection This bit is active for normal and low-power modes. */
    ...
    /** @brief OPAMP calibration output During the calibration mode, the offset is trimmed when this signal toggles. */
    using OPAMP1_CSR_CALOUT = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 15, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief OPAMP high-speed mode This bit is effective for both normal and low-power modes. */
    ...
    /** @brief OPAMP range setting This bit must be set before enabling the OPAMP and this bit affects all OPAMP instances. */
     ...
     */
        /** @brief reserved */
    constexpr std::uint32_t OPAMP1_CSR_OPA_RANGE_B_0x0 = 0;
    ...
    /** @brief Inverting input selection These bits are used only when OPAMODE = 00, 01 or 10. in PGA mode for filtering) 1x: inverting input not externally connected */
    ...
    /** @brief OPAMP calibration output During calibration mode, the offset is trimmed when this signal toggles. */
    using OPAMP2_CRS_CALOUT = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 15, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief OPAMP high-speed mode This bit is effective for both normal and high-speed modes. */
