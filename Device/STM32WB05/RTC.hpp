...
#ifndef EMBEDDED_PP_STM32WB05_RTC_HPP
#define EMBEDDED_PP_STM32WB05_RTC_HPP
...
namespace STM32WB05::RTC {
    ...
    /** @brief Alarm A seconds mask 0: Alarm A set if the seconds match 1: Seconds don't care in Alarm A comparison */
    ...
    /** @brief Alarm A minutes mask 0: Alarm A set if the minutes match 1: Minutes don't care in Alarm A comparison */
    ...
    /** @brief Alarm A hours mask 0: Alarm A set if the hours match 1: Hours don't care in Alarm A comparison */
    ...
    /** @brief Week day selection 0: DU[3:0] represents the date units 1: DU[3:0] represents the week day. DT[1:0] is don't care. */
    using RTC_ALRMAR_WDSEL = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 30, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Alarm A date mask 0: Alarm A set if the date/day match 1: Date/day don't care in Alarm A comparison */
    ...
    /** @brief Sub second value SS[15:0] is the value in the synchronous prescaler's counter. The fraction of a second is given by the formula below: Second fraction = ( PREDIV_S - SS ) / ( PREDIV_S + 1 ) */
    ...
    /** @brief Subtract a fraction of a second These bits are write only and is always read as zero. Writing to this bit has no effect when a shift operation is pending (when SHPF=1, in RTC_ISR). The value which is written to SUBFS is added to the synchronous prescaler's counter. Since this counter counts down, this operation effectively subtracts from (delays) the clock by: Delay (seconds) = SUBFS / ( PREDIV_S + 1 ) A fraction of a second can effectively be added to the clock (advancing the clock) when the ADD1S function is used in conjunction with SUBFS, effectively advancing the clock by : Advance (seconds) = ( 1 - ( SUBFS / ( PREDIV_S + 1 ) ) ) . */
