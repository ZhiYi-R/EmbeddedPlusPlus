...
#ifndef EMBEDDED_PP_STM32WBA54_SEC_ICACHE_HPP
#define EMBEDDED_PP_STM32WBA54_SEC_ICACHE_HPP
...
namespace STM32WBA54::SEC_ICache {
    ...
    /** @brief cache invalidation Set by software and cleared by hardware when the BUSYF flag is set (during cache maintenance operation). Writing 0 has no effect. */
    ...
    /** @brief cache associativity mode selection This bit allows user to choose ICACHE set-associativity. It can be written by software only when cache is disabled (EN=0). */
    ...
    /** @brief interrupt enable on busy end Set by software to enable an interrupt generation at the end of a cache invalidate operation. */
    ...
    /** @brief interrupt enable on cache error Set by software to enable an interrupt generation in case of cache functional error (cacheable write access) */
    ...
    /** @brief clear busy end flag Set by software. */
    ...
    /** @brief base address for region x This alias address is replaced by REMAPADDR field. The only useful bits are [28:RI], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see Section 18.4.7). If the programmed value has more LSBs, the useless bits are ignored. */
    ...
    /** @details Possible values:
     *          - B_0x1: 2Mbytes (value: 1)
     *          - B_0x2: 4Mbytes (value: 2)
     *          - B_0x3: 8Mbytes (value: 3)
     *          - B_0x4: 16Mbytes (value: 4)
     *          - B_0x5: 32Mbytes (value: 5)
     *          - B_0x6: 64Mbytes (value: 6)
     *          - B_0x7: 128Mbytes (value: 7)
     */
        /** @brief 2Mbytes */
    ...
    /** @brief remapped address for region x This field replaces the alias address defined by BASEADDR field. The only useful bits are [31:RI], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see Section 18.4.7). If the programmed value has more LSBs, the useless bits are ignored. */
