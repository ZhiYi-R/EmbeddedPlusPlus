...
#ifndef EMBEDDED_PP_STM32WBA55_SEC_I2C3_HPP
#define EMBEDDED_PP_STM32WBA55_SEC_I2C3_HPP
...
namespace STM32WBA55::SEC_I2C3 {
     ...
     *          - B_0x1: Digital filter enabled and filtering capability up to 1 tsubI2CCLK/sub (value: 1)
     *          - B_0xF: digital filter enabled and filtering capability up to15 tsubI2CCLK/sub (value: 15)
     */
        /** @brief Digital filter disabled */
    constexpr std::uint32_t I2C_CR1_DNF_B_0x0 = 0;
        /** @brief Digital filter enabled and filtering capability up to 1 tsubI2CCLK/sub */
    constexpr std::uint32_t I2C_CR1_DNF_B_0x1 = 1;
        /** @brief digital filter enabled and filtering capability up to15 tsubI2CCLK/sub */
    ...
    /** @brief SCL low period (master mode) This field is used to generate the SCL low period in master mode. tsubSCLL /sub= (SCLL+1) x tsubPRESC/sub Note: SCLL is also used to generate tsubBUF /suband tsubSU:STA /subtimings. */
    using I2C_TIMINGR_SCLL = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 0, 8, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief SCL high period (master mode) This field is used to generate the SCL high period in master mode. tsubSCLH /sub= (SCLH+1) x tsubPRESC/sub Note: SCLH is also used to generate tsubSU:STO /suband tsubHD:STA /subtiming. */
    using I2C_TIMINGR_SCLH = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 8, 8, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Data hold time This field is used to generate the delay tsubSDADEL /subbetween SCL falling edge and SDA edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tsubSDADEL/sub. tsubSDADEL/sub= SDADEL x tsubPRESC/sub Note: SDADEL is used to generate tsubHD:DAT /subtiming. */
    using I2C_TIMINGR_SDADEL = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 16, 4, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Data setup time This field is used to generate a delay tsubSCLDEL /subbetween SDA edge and SCL rising edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tsubSCLDEL/sub. tsubSCLDEL /sub= (SCLDEL+1) x tsubPRESC/sub Note: tsubSCLDEL/sub is used to generate tsubSU:DAT /subtiming. */
    using I2C_TIMINGR_SCLDEL = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 20, 4, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief Timing prescaler This field is used to prescale i2c_ker_ck in order to generate the clock period tsubPRESC /subused for data setup and hold counters (refer to FMPI2C timings on page 1928) and for SCL high and low level counters (refer to FMPI2C master initialization on page 1951). tsubPRESC /sub= (PRESC+1) x tsubI2CCLK/sub */
    ...
    /** @brief Bus Timeout A This field is used to configure: The SCL low timeout condition tsubTIMEOUT/sub when TIDLE=0 tsubTIMEOUT/sub= (TIMEOUTA+1) x 2048 x tsubI2CCLK/sub The bus idle condition (both SCL and SDA high) when TIDLE=1 tsubIDLE/sub= (TIMEOUTA+1) x 4 x tsubI2CCLK/sub Note: These bits can be written only when TIMOUTEN=0. */
     ...
     *          - B_0x1: SCL timeout detection is enabled: when SCL is low for more than tsubTIMEOUT/sub (TIDLE=0) or high for more than tsubIDLE /sub(TIDLE=1), a timeout error is detected (TIMEOUT=1). (value: 1)
     */
        /** @brief SCL timeout detection is disabled */
    constexpr std::uint32_t I2C_TIMEOUTR_TIMOUTEN_B_0x0 = 0;
        /** @brief SCL timeout detection is enabled: when SCL is low for more than tsubTIMEOUT/sub (TIDLE=0) or high for more than tsubIDLE /sub(TIDLE=1), a timeout error is detected (TIMEOUT=1). */
     ...
     *          - B_0x1: Extended clock timeout detection is enabled. When a cumulative SCL stretch for more than tsubLOW:EXT /subis done by the I2C interface, a timeout error is detected (TIMEOUT=1). (value: 1)
     */
        /** @brief Extended clock timeout detection is disabled */
    constexpr std::uint32_t I2C_TIMEOUTR_TEXTEN_B_0x0 = 0;
        /** @brief Extended clock timeout detection is enabled. When a cumulative SCL stretch for more than tsubLOW:EXT /subis done by the I2C interface, a timeout error is detected (TIMEOUT=1). */
    ...
    /** @brief Timeout or tsubLOW/sub detection flag This flag is set by hardware when a timeout or extended clock timeout occurred. It is cleared by software by setting the TIMEOUTCF bit. Note: This bit is cleared by hardware when PE = 0. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation. */
    ...
    /** @brief 8-bit receive data Data byte received from the Isup2/supC bus */
    ...
    /** @brief 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1. */
    ...
    /** @brief Trigger selection (refer to Section 52.4.3: FMPI2C pins and internal signals I2C interconnections tables). ... Note: This bit can be written only when PE = 0 */
    ...
    /** @brief Trigger polarity Note: This bit can be written only when PE = 0 */
    ...
    /** @brief Trigger enable When a trigger is detected, a START condition is sent and the transfer is launched as defined in I2C_CR2. */
