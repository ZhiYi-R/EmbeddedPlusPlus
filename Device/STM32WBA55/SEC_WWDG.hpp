...
#ifndef EMBEDDED_PP_STM32WBA55_SEC_WWDG_HPP
#define EMBEDDED_PP_STM32WBA55_SEC_WWDG_HPP
...
namespace STM32WBA55::SEC_WWDG {
    ...
    /** @brief 7-bit counter (MSB to LSB) These bits contain the value of the watchdog counter, decremented every (4096 x 2supWDGTB[2:0]/sup) PCLK cycles. A reset is produced when it is decremented from 0x40 to 0x3F (T6 becomes cleared). */
    ...
    /** @brief Early wakeup interrupt flag This bit is set by hardware when the counter has reached the value 0x40. It must be cleared by software by writing 0. Writing 1 has no effect. This bit is also set if the interrupt is not enabled. */
