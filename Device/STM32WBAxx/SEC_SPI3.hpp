...
#ifndef EMBEDDED_PP_STM32WBAXX_SEC_SPI3_HPP
#define EMBEDDED_PP_STM32WBAXX_SEC_SPI3_HPP
...
namespace STM32WBAxx::SEC_SPI3 {
    ...
    /** @brief FIFO threshold level Defines number of data frames at single data packet. Size of the packet should not exceed 1/2 of FIFO space. SPI interface is more efficient if configured packet sizes are aligned with data register access parallelism: If SPI data register is accessed as a 16-bit register and DSIZE ¤ 8 bit, better to select FTHLV = 2, 4, 6. If SPI data register is accessed as a 32-bit register and DSIZE> 8 bit, better to select FTHLV = 2, 4, 6, while if DSIZE ¤ 8bit, better to select FTHLV = 4, 8, 12. Note: FTHLV[3:2] bits are reserved at instances with limited set of features */
    ...
    /** @details Possible values:
     *          - B_0x0: RxFIFO is empty or a not complete data packet is received (value: 0)
     *          - B_0x1: RxFIFO contains at least 1 data packet (value: 1)
     */
        /** @brief RxFIFO is empty or a not complete data packet is received */
    constexpr std::uint32_t SPI_SR_RXP_B_0x0 = 0;
        /** @brief RxFIFO contains at least 1 data packet */
    constexpr std::uint32_t SPI_SR_RXP_B_0x1 = 1;

    /** @brief Tx-Packet space available TXP flag is changed by hardware. It monitors overall space currently available at TxFIFO no matter if SPI is enabled or not. It has to be checked once a complete data packet is stored at TxFIFO. */
    using SPI_SR_TXP = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 1, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @details Possible values:
     *          - B_0x0: there is not enough space to locate next data packet at TxFIFO (value: 0)
     *          - B_0x1: TxFIFO has enough free location to host 1 data packet (value: 1)
     */
        /** @brief there is not enough space to locate next data packet at TxFIFO */
    constexpr std::uint32_t SPI_SR_TXP_B_0x0 = 0;
        /** @brief TxFIFO has enough free location to host 1 data packet */
    ...
    /** @brief underrun at slave transmission mode This bit is cleared by writing 1 to UDRC bit at SPI_IFCR Note: UDR flag applies to Slave mode only */
    ...
    /** @brief overrun This bit is cleared by writing 1 to OVRC bit at SPI_IFCR */
    ...
    /** @brief CRC error This bit is cleared by writing 1 to CRCEC bit at SPI_IFCR */
    ...
    /** @brief TI frame format error This bit is cleared by writing 1 to TIFREC bit at SPI_IFCR */
    ...
    /** @brief mode fault This bit is cleared by writing 1 to MODFC bit at SPI_IFCR */
     ...
     *          - B_0x1: mode fault detected. When MODF is set, SPE and IOLOCK bits at SPI_CR1 register are reset and their setting is blocked. (value: 1)
     */
        /** @brief no mode fault */
    constexpr std::uint32_t SPI_SR_MODF_B_0x0 = 0;
        /** @brief mode fault detected. When MODF is set, SPE and IOLOCK bits at SPI_CR1 register are reset and their setting is blocked. */
    ...
    /** @brief number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus or during autonomous operation at low-power mode. Note: CTSIZE[15:0] bits are not available at instances with limited set of features */
    using SPI_SR_CTSIZE = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 16, 16, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @brief SPI_IFCR register */
    ...
    /** @brief trigger of CSTART control enable Note: if user can't prevent trigger event during write, the TRIGEN has to be changed when SPI is disabled */
    using SPI_AUTOCR_TRIGEN = ::EmbeddedPP::LowLevel::BitField<std::uint32_t, 21, 1, ::EmbeddedPP::LowLevel::Accessibility::ReadWrite>;
    /** @details Possible values:
     *          - B_0x0: trigger of CSTART control disabled (value: 0)
     *          - B_0x1: trigger of CSTART control enabled (value: 1)
     */
        /** @brief trigger of CSTART control disabled */
    constexpr std::uint32_t SPI_AUTOCR_TRIGEN_B_0x0 = 0;
        /** @brief trigger of CSTART control enabled */
    constexpr std::uint32_t SPI_AUTOCR_TRIGEN_B_0x1 = 1;

    /** @brief SPI_TXDR register */
